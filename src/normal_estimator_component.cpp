...
void NormalEstimatorComponent::cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  std::cout << "time: " <<
    ...
    std::milli>(end_time - start_time).count() << "[s]" << std::endl;
}
...
void NormalEstimatorComponent::estimate_normal(CloudXYZINPtr & cloud)
{
  ...
  for (int i = 0; i < size; i += skip_) {
    ...
    third_component_index = eigen_values(third_component_index) >
      eigen_values(1) ? 1 : third_component_index;
    third_component_index = eigen_values(third_component_index) >
      eigen_values(2) ? 2 : third_component_index;
    ...
    cloud->points[i].curvature = eigen_values(third_component_index) /
...
void NormalEstimatorComponent::get_d_gaussian_sphere(
  const CloudXYZINPtr & cloud,
  CloudXYZINPtr & d_gaussian_sphere)
{
