...
size_t skip = 5;
float skip_ = 1.0/skip;	
int HNN = 5;
int HNN_VNN = (2*HNN+1)*(2*VNN+1);
float HNN_VNN_ = 1.0/HNN_VNN;	
float HNN2VNN2_ = 1.0/((2*(float)HNN)*(2*(float)VNN));
float VR = 0.839 * 0.60;
float MIN_RANGE = 0.2f;
float MIN_RANGE2 = (MIN_RANGE*MIN_RANGE);
float MAX_RANGE = 120.0f;
float MAX_RANGE2 = (MAX_RANGE*MAX_RANGE);
...
////clock_t startTime, stopTime, countTime=1, diffTime;
////double aveTime=0;
	...
	for(size_t i=0;i<i_end;i+=skip){
		...
		for(int j=-VNN;j<=VNN;j++){
			...
			for(int k=-HNN;k<=HNN;k++){
				...
				//if(!jud(i_j+32*k)) continue;	// 20151104
				if(!jud(ij32k)) continue;	// 20151104
				//size_t num_tmp = itr(i+j+32*k);
				//size_t num_tmp = itr(i_j+32*k);	// 20151104
				...
				//if(( norm_vh < vector_horizon*p_q.norm() ) && ( fabs((int)(i%VN)-(int)((ij32k)%VN)) ) <= VNN ){	// 20151104
				//if(( norm_vh < vhpq ) && ( fabs((int)(i%VN)-(int)((ij32k)%VN)) ) <= VNN ){	// 20151104
